...
public static partial class ApiHandlers
{
        ...
        await using var command = new NpgsqlCommand("""
            SELECT j.parchment_id,j.state,j.resume_stage,j.spell_id,j.attempt_count,j.message,j.error_code,j.retryable,
                   ...
                   EXISTS(SELECT 1 FROM visual_atlases atlas
                       ...
                       JOIN provider_attempts pa ON pa.id=atlas.provider_attempt_id AND pa.job_id=j.id
                           AND pa.stage='G' AND pa.status='success' AND pa.output_sha256=native.sha256
                           AND pa.input_sha256=atlas.input_sha256
                       JOIN interpretations interpreted ON interpreted.job_id=j.id
                           AND interpreted.description_sha256=atlas.description_sha256
                       JOIN artifacts description ON description.id=interpreted.description_artifact_id
        ...
        ApiConfig config, IArtifactStore store, CancellationToken ct)
    {
        ...
        int attempts, attemptBudget, pipelineVersion;
        long bAttempts;
            ...
            """, connection, transaction))
        {
            ...
            pipelineVersion = reader.GetInt32(14);
            attemptBudget = pipelineVersion == 5 ? V2ConstructionPolicy.AttemptBudget : 10;
        }
        if (state != "ready")
        {
            ...
            await using var update = new NpgsqlCommand("""
                UPDATE jobs SET state='queued',
                    ...
                    v2_revision_base=CASE WHEN @owner_construction_retry THEN @revision_base ELSE v2_revision_base END,
                    v2_builder_version=CASE WHEN @owner_construction_retry THEN @builder_version ELSE v2_builder_version END,
                    next_attempt_at=NULL,lease_until=NULL,leased_by=NULL,error_code=NULL,retryable=false,
                    message='Reprise demandée',updated_at=now()
            ...
            if (kind == "production" && parchmentId is Guid resumedParchment)
            {
                await using var parchment = new NpgsqlCommand("UPDATE parchments SET state='processing',updated_at=now() WHERE id=@id AND owner_id=@owner AND state='incident'", connection, transaction);
                parchment.Parameters.AddWithValue("id", resumedParchment);
