...
4. Appliquer `backend/migrations/003_player_invitations.sql` au laboratoire de déploiement après sauvegarde ; la migration a déjà été appliquée et le nouvel échange testé 4/4 sur `palimpseste_test` uniquement. Un nouvel invité ne doit jamais être ouvert sans invitation privée.
