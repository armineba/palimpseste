...
Le backend contenant les recettes est déployé sur ce PC : API et worker sous `PalRuntimeSvc`, santé HTTP 200 et doctor local sans problème de production. L'archive opérateur a été régénérée. **Prochaine preuve :** un nouveau dessin doit montrer une sélection Astra d'ID(s) de recette, un plan Luna accepté, un paquet téléchargé et son lancement dans le laboratoire. Aucun essai modèle réel avec le catalogue des 141 recettes n'est encore consigné. Ne pas compter la compilation des 705 couples comme une recette humaine. Les décisions des créateurs sur fidélité, VFX et son, les 30 dessins inédits, le second créateur et l'autre PC restent ouverts. La section suivante est l'état historique antérieur aux recettes.
