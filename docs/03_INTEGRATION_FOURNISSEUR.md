...
> **Priorité actuelle D14 :** A décrit obligatoirement apparition, activité, contact et expiration pour chaque sujet. G produit l'image cible ; B traduit la description et l'image en plan borné ; le service capture quatre phases avec un **Player Unity précompilé**, puis appelle un **nouveau critique J indépendant** et, si nécessaire, B pour corriger uniquement les profils visuels. Cette boucle Dream-loop Pro conserve les versions et les verdicts et reste bornée. A `gpt-5.6-sol` / `high`, G/B/J `gpt-6-astra` / `high` ; prompts A `2.3`, B `2.2`, J `1.0`. La description prime pour la chronologie ; l'image vise le moment actif. Voir [D14](DECISIONS.md).
>
> La migration `009_visual_review.sql` ajoute la persistance de revue et la nouvelle version du parcours. Le service déclenche une action de rendu fixe, autorisée par le créateur lors de sa génération ; aucun modèle ne reçoit cet outil ni un accès à la compilation logicielle. Le binaire Codex durci garde A/B/J sans outil exécutable et G limité à l'image native. Les reprises ne peuvent changer les mécaniques ni exécuter du code généré. Les captures décoratives ne prouvent ni collisions/dégâts, ni fluidité, ni son.
>
> Pour cette livraison, **aucun nouveau test, appel modèle ou capture de validation n'est lancé par le développement**, conformément à la demande de laisser le créateur essayer. Build et packaging Player `1.4.0` réussis ; backend D14 déployé localement, migration `009` appliquée, renderer protégé installé et jeu ouvert. [Preuve locale](../evidence/public/backend/lifecycle-2026-09-20.json) et [état de réalisation](IMPLEMENTATION_STATUS.md). Le parcours joueur et les jugements D14 restent à observer ; les preuves D13 suivantes ne les valident pas.

> **Historique D13, 20 septembre 2026 :** le parcours obligatoire des nouveaux sorts est A description → G vraie image → B composition 3D/VFX depuis l'image. Les réglages A/B de D10 restent A `gpt-5.6-sol` / `high` (prompt `2.2`) et B `gpt-6-astra` / `high` (prompt `2.1`, multimodal). G utilise `gpt-6-astra` / `high`, l'outil natif `image_generation` et l'abonnement Codex connecté, sans clé API. `PALIMPSESTE_CODEX_EXE` et `PALIMPSESTE_IMAGE_CODEX_EXE` doivent désigner le même nouveau `codex-image.exe`. Avec `PALIMPSESTE_IMAGEGEN_TEXT_ONLY=1`, son filtre natif vide le registre A/B et ne conserve que l'outil image en G ; il corrige l'exposition de `apply_patch` observée dans l'ancien CLI malgré les flags désactivés. L'ancien fichier et ses preuves restent des archives. Exiger trois rapports au SHA durci : `provider_doctor/local` pour les fonctions, `interpreter_multimodal_probe` pour A, `provider_image_generation_doctor` pour B et G avec leurs hashs liés. Voir [D13](DECISIONS.md) et [l'état de réalisation](IMPLEMENTATION_STATUS.md). La révision B `2.1` précise `turn_mdeg_s=0` hors `homing`, après le rejet réel `turn_rate` ; la reprise de B seul a ensuite validé le plan et compilé le paquet avec le même PNG G.
>
> D13 remplace les passages historiques interdisant une génération d'image par joueur ou les images en entrée B. G produit un PNG de 512 à 2048 pixels par dimension, au plus 8 Mio, enregistré dans un checkpoint durable ; B reçoit ce fichier réel et son hash, avec la description figée et les capacités. `appearance.construction` contient uniquement des parties, matériaux et mouvements bornés ; la référence est liée à la description et au plan, séparée des masques mécaniques et exige le client `1.3.0`. Une reprise réutilise l'image figée. Les schémas stricts, l'isolation runtime, le contrôle de propriétaire et les bornes d'appels restent applicables. Aucun test ni build logiciel n'est ajouté à la génération joueur ; aucun code généré n'est exécuté. Les durées exposées sont mesurées. Le transport G puis B repris ont réussi ; le service D13 est déployé localement et le Player `1.3.0` ouvert. Le nouveau parcours joueur complet et son acceptation restent à constater.
