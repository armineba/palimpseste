...
> **Priorité actuelle D14 :** la description pilote les quatre phases de chaque sujet — apparition, activité, contact et disparition naturelle — et l'image générée fixe la cible du moment actif. La boucle **Dream-loop Pro** ajoute des captures par un Player Unity précompilé et une nouvelle session critique indépendante J, puis des corrections bornées du plan. Les modèles restent A `gpt-5.6-sol` / `high` ; G/B/J `gpt-6-astra` / `high`. Prompts A `2.3`, B `2.2`, J `1.0` ; client `1.4.0`. Voir [D14](DECISIONS.md).
>
> L'autorisation utilisateur porte sur ces captures et critiques **runtime lors de sa génération**. Elle remplace les restrictions historiques interdisant les captures par job. Le renderer est fixé par le développement ; les modèles A/B/J restent sans outil exécutable et G ne dispose que de l'image native. Aucun modèle ne lance de capture ou de build, n'écrit de code ou n'accède aux secrets. Les quatre captures de présentation ne prouvent pas les mécaniques, la fluidité ou le son.
>
> Le créateur demande maintenant à tester lui-même : **l'agent de développement ne lance plus de tests, appels modèle ni captures de validation pour cette livraison**. Build et packaging Player `1.4.0` réussis, raccourci actualisé et jeu ouvert ; backend D14 déployé localement, migration `009` appliquée et renderer protégé installé. Aucun parcours joueur D14 ni verdict artistique n'est encore constaté. [Preuve locale](../evidence/public/backend/lifecycle-2026-09-20.json), [état réel](IMPLEMENTATION_STATUS.md) et [reprise](NEXT_ACTIONS.md).

> **Historique D13, 20 septembre 2026 :** l'auteur demande pour chaque nouveau sort une vraie image générée depuis la description, puis sa reconstruction 3D/VFX. A reste `gpt-5.6-sol` / `high`, prompt `2.2` ; le nouveau stade G utilise `gpt-6-astra` / `high` et l'outil natif `image_generation` de Codex avec l'abonnement connecté, sans clé API ; B reste `gpt-6-astra` / `high`, reçoit cette image et utilise le prompt `2.1`. Voir [D13](DECISIONS.md). Aucun changement implicite des modèles A/B n'est autorisé par le nom courant donné à l'interprète. La révision B `2.1` précise `turn_mdeg_s=0` hors `homing`, après le rejet réel `turn_rate` ; la reprise de B seul a ensuite validé le plan et compilé le paquet avec le même PNG G.
>
