...
> **Priorité actuelle D10, 20 septembre 2026 :** A utilise `gpt-5.6-sol` / `high` avec le prompt `2.2` ; B utilise `gpt-6-astra` / `high` avec le prompt `1.9`. Cette dernière demande remplace les anciens choix de modèles/efforts ci-dessous. Le profil facultatif `appearance.vfx` compose des couches visuelles bornées dans Unity, sans modifier les mécaniques ni fournir du code ; sa présence exige le client `1.2.0`. Voir [D10](DECISIONS.md). Le dessin libre, les formes 3D interprétées et l'isolation `codex exec` restent applicables.
>
> Le code réduit les projections et la taille des schémas envoyés, et expose le temps écoulé réel. Ni un gain de latence, ni l'activation du nouveau couple, ni un build `1.2.0` ne sont établis par cet avenant : les preuves et le point de reprise sont dans [l'état de réalisation](IMPLEMENTATION_STATUS.md). Les tests et builds appartiennent au développement, jamais au parcours de génération joueur.
