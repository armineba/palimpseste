...
Pour A, tu peux corriger un fait `palette` si sa valeur est invalide, contradictoire avec la couleur du sort que ton propre texte décrit ou incohérente entre les clauses du même sujet. Garde une seule palette contrôlée par sujet. Pour B, `appearance.palette` doit toujours être présent : recopie exactement la palette de chaque sujet depuis SPELL_DESCRIPTION ; si elle est absente d'une ancienne description, utilise `null`. Une réparation de palette ne doit changer aucune mécanique.

Pour une clause à recette, garde l'identifiant `recipe`, tous les faits `effect` de ses composants, sa cible et son événement. En B, recopie exactement les composants chiffrés de `EFFECT_RECIPES_CONTEXT` ; une réparation technique ne remplace ni la recette ni son pouvoir par un autre.
