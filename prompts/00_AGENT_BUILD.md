...
>
> Le créateur autorise le service à capturer le rendu lors de sa génération pour cette boucle Pro. Cette action runtime fixe n'est pas un outil donné au modèle : aucun code produit, build logiciel, shell ou accès aux secrets. A/B/J utilisent le CLI durci sans outil exécutable ; seul G reçoit l'image native. Conserver la mécanique lors des corrections visuelles et borner toute itération. Une image ou un score J ne prouve ni fluidité, ni son, ni acceptation humaine.
>
> **Consigne de livraison actuelle : laisser le créateur tester lui-même. Ne plus lancer de tests, appels modèle ou captures de validation depuis l'agent de développement.** Le CoreSmoke antérieur à cette consigne et le build/packaging `1.4.0` réussis sont des faits distincts. Raccourci actualisé, jeu ouvert ; backend D14 déployé localement, migration `009` appliquée et renderer protégé installé. [Preuve locale](../evidence/public/backend/lifecycle-2026-09-20.json) et [état réel](../docs/IMPLEMENTATION_STATUS.md). Aucun parcours joueur D14 ni verdict visuel n'est encore constaté ; ne pas extrapoler les preuves D13.

> **Historique D13, 20 septembre 2026 :** chaque nouveau sort suit dessin → description → vraie image générée → reconstruction 3D/VFX depuis cette image → compilation contrôlée. A reste `gpt-5.6-sol` / `high` (`sp.prompt.a/2.2`) ; G utilise `gpt-6-astra` / `high` avec l'outil natif `image_generation` de Codex ; B reste `gpt-6-astra` / `high`, désormais multimodal avec `sp.prompt.b/2.1` (consigne explicite `turn_mdeg_s=0` hors `homing`, après le rejet réel `turn_rate`). G utilise l'abonnement connecté, sans clé API. A/B/G doivent sélectionner le même nouveau `codex-image.exe`, dont le filtre natif vide le registre d'outils A/B et ne conserve que l'image en G. L'ancien CLI expose encore `apply_patch` malgré les flags désactivés ; son fichier et ses preuves restent des archives, sans valider le nouveau binaire. Exiger de nouvelles preuves locale, A, puis G+B sur son SHA. Voir [D13](../docs/DECISIONS.md) et l'[état réellement prouvé](../docs/IMPLEMENTATION_STATUS.md).
>
> D13 remplace les interdictions D10–D12 de générer une image par joueur et les passages historiques disant que B ne reçoit aucune image. L'image figée et son hash alimentent une composition déclarative bornée `appearance.construction`, vérifiée par le compilateur et exigeant le client `1.3.0`. Les anciens paquets restent lisibles. Aucun test ou build logiciel n'appartient au parcours joueur ; aucun code généré ne s'exécute. Mesurer les étapes A/G/B réelles, conserver leurs checkpoints et distinguer preuve technique et acceptation artistique. À ce point, D13 n'est pas déclaré activé en production.
