...
## Mise à jour D16 / 1.6.0

Les scripts sont préparés pour **`WindowsAnimationSheetRelease` → `WindowsAnimationSheetPlayable`**, journal `game/Logs/animation-sheet-build.log`. Cette préparation ne constitue pas une preuve de build ou de déploiement ; consulter [l'état réel](../docs/IMPLEMENTATION_STATUS.md).

- `package-lifecycle-player.ps1` attend un build réussi `1.6.0`. Il conserve le chemin historique `evidence/public/unity/lifecycle-delivery.json`, avec version et nom D16 explicites, sauvegarde la livraison précédente et ajoute les deux notices CC0 des textures ainsi que la licence MIT du bruit HLSL sous `ThirdPartyNotices/`. Le manifeste distingue fichiers du build et licences ajoutées au packaging. Le script n'ouvre jamais le jeu.
- `package-backend.ps1` publie les programmes et inclut les migrations `001` à `011`. La spécification exporte `assets/sourced-vfx/catalogue.json`, `references.json`, les licences et les PNG. Les historiques `01_MODEL_A_INTERPRETE_2_3.md` et `04_IMAGE_REFERENCE_1_2.md` restent disponibles pour les anciens parcours. La notice MIT de System.Drawing.Common accompagne les programmes publiés. Les exemples HLSL `reference-code` et scripts d'import restent hors de la spécification runtime.
- `deploy-lifecycle.ps1 -StageRoot <stage-publié>` installe le Player protégé `1.6.0` et met à jour **le laboratoire D15 existant**. Il vérifie la table `spell_reference_research` apportée par `010`, puis applique **uniquement `011_animation_sheet.sql`** provenant du stage, après arrêt de l'admission et du worker sans job actif. Le renderer installé utilise un répertoire protégé `render-animation-sheet-*`. Le script ne lance aucun diagnostic, modèle ou essai de jeu.
- `refresh-backend-delivery.ps1 -StageRoot <stage-publié>` actualise documentation et preuves autour des binaires déjà publiés. `-ProofPath` est facultatif et désigne par défaut `evidence/public/backend/animation-sheet-d16.json`. Les SHA des binaires doivent correspondre à cette preuve ; sa version alimente l'index historique `evidence/public/backend/lifecycle-delivery.json`. Une preuve D15 peut être donnée explicitement pour un ancien stage.

**Ne pas rejouer `009_visual_review.sql` ou `010_behavior_research.sql` sur des parcours plus récents** : leurs contraintes excluent les nouvelles versions de jobs. Pour une **base neuve**, l'opérateur applique une seule fois toutes les migrations `001` à `011` dans l'ordre des noms, après préparation de sa connexion privée. Le script de mise à jour ci-dessus ne remplace pas cette initialisation complète. Les archives conservent les migrations triées pour cette installation.

La cible D16 est une planche **3 lignes × 7 cases**, APPARITION / STABLE / DISPARITION, numérotées de 1 à 7. G produit un atlas sans marges ; le compositeur serveur fixe ajoute cadres, titres et repères bleu/vert/violet sur fond sombre, en **1536 × 1152**. L'atlas natif et la planche composée conservent des artefacts et SHA distincts. Le compositeur Windows utilise System.Drawing.Common gratuit sous MIT et les polices système ; il n'est pas un outil accessible au modèle.
