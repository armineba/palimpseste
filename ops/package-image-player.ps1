...
$source = Join-Path $buildRoot 'WindowsImageReferenceRelease'
$playable = Join-Path $buildRoot 'WindowsImageReferencePlayable'
$zipPath = Join-Path $repo 'deliverables\Palimpseste-Windows-x64-IL2CPP.zip'
$proofPath = Join-Path $repo 'evidence\public\unity\image-reference-delivery.json'
$logPath = Join-Path $repo 'game\Logs\image-reference-build-final.log'
$stamp = [DateTime]::UtcNow.ToString('yyyyMMdd-HHmmss') + '-' + [Guid]::NewGuid().ToString('N').Substring(0, 8)
$stage = Join-Path $buildRoot ('image-package-stage-' + $stamp)
$pendingZip = $stage + '.zip'
$backup = Join-Path $buildRoot ('ImageReferenceBackups\' + $stamp)
...
if ([IO.File]::ReadAllText((Join-Path $repo 'game\ProjectSettings\ProjectSettings.asset')) -notmatch '(?m)^\s*bundleVersion:\s*1\.3\.0\s*$') { throw 'La version du projet doit être 1.3.0.' }
...
$proof = [ordered]@{
    observed_at = [DateTime]::UtcNow.ToString('O'); client_version = '1.3.0'; unity = '6000.3.24f1'; backend = 'IL2CPP'
    ...
    model_calls = 0; art_acceptance = 'not_claimed'; player_launch = $null
}
Save-Proof $proof
...
if ($Launch) {
    ...
    if (Test-Path -LiteralPath $shortcutPath) { Copy-Item -LiteralPath $shortcutPath -Destination (Join-Path $backup 'Palimpseste Spell Lab.lnk') }
    $shell = New-Object -ComObject WScript.Shell
    $shortcut = $shell.CreateShortcut($shortcutPath)
    $shortcut.TargetPath = $executable; $shortcut.WorkingDirectory = $playable
    $shortcut.Description = 'Palimpseste Spell Lab 1.3.0'; $shortcut.Save()
    $player = Start-Process -FilePath $executable -WorkingDirectory $playable -WindowStyle Normal -PassThru
    $proof.player_launch = [ordered]@{ pid = $player.Id; requested_at = [DateTime]::UtcNow.ToString('O'); shortcut_updated = $true }
    Save-Proof $proof
}
Write-Output "Player 1.3.0 : $playable"
